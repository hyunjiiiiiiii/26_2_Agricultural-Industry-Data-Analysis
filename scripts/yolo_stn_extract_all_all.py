"""
YOLO(det) -> (physiological only) crop -> STN(classify) feature extractor

✅ 수정 사항(요청 반영):
- disease/pesticide 세부 구분 없이,
  YOLO에서 physiological(class=phys_id) 이외의 탐지가 있었는지 여부만 이미지 단위로 기록
  => has_other_det, has_any_det 추가
- STN은 기존처럼 physiological만 수행
- boxes CSV는 physiological 박스만 기록 (기존 유지)

사용 예:
python scripts/yolo_stn_extract_all_all.py \
  --source dataset/images/test \
  --yolo runs/detect/train2/weights/best.pt \
  --stn runs_cls/runs_cls/stn_resnet18/best.pth \
  --conf 0.12 --out outputs/val --save_crops
"""
...
def main():
    ...
    # 이미지 단위 컬럼(모든 이미지)
    # ✅ 추가: has_any_det, has_other_det
    img_fields = [
        "id", "image_path", "image_name",
        "has_any_det",           # 0/1 (YOLO가 뭐라도 잡았는지)
        "has_phys_det",          # 0/1 (phys_id 탐지)
        "has_other_det",         # 0/1 (phys_id 이외 탐지)
        "num_phys_boxes",
        "yolo_conf_max", "yolo_conf_mean",
        "stn_valid",             # 0/1 (STN을 실제로 수행했는지)
        "stn_pmax_max", "stn_pmax_mean",
        "stn_entropy_mean",
        "yolo_det_classes",        # 예: "0|1|5"  (이미지에서 탐지된 class id 집합)
        "yolo_det_classes_names",  # 예: "disease|phys|pesticide" (가능할 때)
    ...
    for idx, im_path in enumerate(images, start=1):
        ...
        has_any_det = 1
        phys_idxs = [i for i in range(len(clss)) if clss[i] == args.phys_id]
        other_idxs = [i for i in range(len(clss)) if clss[i] != args.phys_id]
        has_other_det = 1 if len(other_idxs) > 0 else 0

        # physiological이 없으면 STN은 skip, 하지만 has_other_det/has_any_det는 기록
        if len(phys_idxs) == 0:
            empty_row = {
                "id": img_id,
                "image_path": str(im_path),
        ...
        for b_i, j in enumerate(phys_idxs):
            x1, y1, x2, y2 = xyxy[j]
    ...
    with open(boxes_csv, "w", newline="", encoding="utf-8") as f:
        ...
        wri.writeheader()
        for rr in box_rows:
            wri.writerow({k: rr.get(k, "") for k in box_fields})

    with open(images_csv, "w", newline="", encoding="utf-8") as f:
        wri = csv.DictWriter(f, fieldnames=img_fields)
        wri.writeheader()
        for rr in img_rows:
            wri.writerow({k: rr.get(k, 0) for k in img_fields})

    print("\nDone.")
    print(f"- images(all): {images_csv}")
    print(f"- boxes(phys det only): {boxes_csv}")
    if args.save_crops:
